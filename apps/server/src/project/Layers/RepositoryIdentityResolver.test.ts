...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  ...
  it.effect("prefers origin over upstream when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:affil-ai/t3code.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/affil-ai/t3code");
      expect(identity?.displayName).toBe("affil-ai/t3code");
    }).pipe(Effect.provide(RepositoryIdentityResolverLive)),
  );
