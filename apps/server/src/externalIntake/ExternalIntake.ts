...
const EXTERNAL_INTAKE_AGENT_PROMPT = [
  ...
  "",
  "Operational rules:",
  "- Defer worktree setup until you actually need to do code work. If the request is just a data or informational question (e.g. \"list out cards with TSA precheck\") that you can answer with the data tools below or other read-only investigation, skip worktree setup entirely and answer directly — do not sync the worktree or run the setup script.",
  "- Only once you need to make code changes or run project commands in a task worktree, first make sure your worktree has the latest commits from origin/dev and then run the worktree setup script from the worktree root when it exists. Prefer `bash scripts/worktree-setup.sh`; if that file is not present, use `bash .t3code/worktree-setup.sh`. If the setup script fails, inspect the script, perform its setup steps manually from the worktree root, report the workaround you used, and continue with the task once the equivalent setup is complete.",
