...
function rawProperty(raw: unknown, key: string) {
...
export function slackConversationKind(input: {
  ...
  const channelType =
    input.raw === undefined
      ? undefined
      : (input.raw as unknown as Record<string, unknown>)["channel_type"];
...
function firstNonEmpty(...values: ReadonlyArray<string | undefined>) {
...
function slackMessageRef(thread: Thread, message: Message) {
  ...
  const threadTs = firstNonEmpty(
    rawProperty(raw, "thread_ts"),
    tsFromThread,
    rawProperty(raw, "ts"),
    message.id,
  );
  if (threadTs === undefined) {
    throw new Error(`Unable to resolve Slack thread timestamp for message ${message.id}`);
  }
...
export function canonicalSlackChatThreadId(thread: Thread, message: Message) {
  const ref = slackMessageRef(thread, message);
...
const makeExternalChat = Effect.gen(function* () {
  ...
  bot.onDirectMessage(async (thread, message, _channel, context) => {
    await runPromise(
      processSlackMessage({
        thread: bot.thread(canonicalSlackChatThreadId(thread, message)),
        message,
        context,
      }),
    );
